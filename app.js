...
*/
const startPokemon = pokemon.find((character) => character.starter === true);
...
*/
const evolvedPokemon = pokemon[startPokemon.number + 1];
...
game.party.forEach(character => console.log(character.name));
...
pokemon.forEach(character => {
    if(character.starter) console.log(character.name);
});
...
let pokeballItem = game.items.find(item => item.name === "pokeball");
...
game.gyms.forEach(gym => {
    if(gym.difficulty < 9) gym.completed = true;
});
game.gyms.gymStatus();
